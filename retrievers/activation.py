...
Retrieval is a presentation: returned memories have current_time appended
to their presentation_log."""
...
from activation.params import W, TAU, LAMBDA, NOISE_SIGMA, D_BASE
...
class ActivationRetriever:
    ...
    def __init__(self, seed: int, w=None, tau=TAU, lam=LAMBDA,
                 noise_sigma=NOISE_SIGMA, reinforce=True, d_base=D_BASE):
    ...
    def retrieve(self, query: str, ctx: Context, k: int) -> list[ScoredMemory]:
        ...
        for mem_id, text, meta, emb in zip(
            ...
            b = base_level(log, ctx.current_time, salience, self.lam, self.d_base)
